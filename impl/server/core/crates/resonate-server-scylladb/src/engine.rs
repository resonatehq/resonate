//! This server's own internals: what a transition takes and what it returns.
//!
//! A resemblance to the SQL servers' engine, not a dependency on it. The same
//! shape turned out to suit both, and copying it is what keeps them free to
//! diverge — ScyllaDB is EAGER where the relational engines are lazy, and that
//! difference has further to run.
//!
...
// The variant names track the schema — four kinds are four columns — which is
// worth more than avoiding the repetition of the enum's own name.
