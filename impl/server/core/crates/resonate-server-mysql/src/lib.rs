    ...
    fn task_fence_create(&self, params: &TaskFenceCreateParams) -> StorageResult<TaskFenceResult> {
        ...
        let promise = if fence_ok {
            ...
            if res.rows_affected() > 0 {
                // The INSERT put the row on the queue if it is pending and
                // external; that is the deadline to announce, task or no task.
                if !already_timedout && resonate_sql::external_tags(tags) {
                    self.arm_promise_timeout(promise_id, timeout_at);
                }
    ...
    fn process_schedule_timeout(
        ...
        if promise_res.rows_affected() > 0 {
            ...
            if !already_timedout && resonate_core::types::is_external(promise_tags) {
                self.arm_promise_timeout(&computed_promise_id, computed_timeout_at);
            }
...
/// This server, as a plugin. The one thing a binary names to run on MySQL.
pub static PLUGIN: ServerPlugin = ServerPlugin::new(env!("CARGO_PKG_NAME"), configure);

/// Everything under `[servers.server_mysql]`.
...
/// Read `[servers.server_mysql]` and build the server. Nothing is opened here — the
...
fn configure(
    ...
    Ok(resonate_sql::server::Server::new(
        Box::new(move |debug| {
            Box::pin(async move {
                let engine = MysqlEngine::connect(
                    &open.url,
                    open.pool_size,
                    open.retry_timeout,
                    open.preload_limit,
