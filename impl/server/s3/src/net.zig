...
pub const Server = struct {
    ...
    closing: bool = false,
    /// Whether the listening socket has been closed, so nothing closes it twice.
    listener_closed: bool = false,
    /// The cancellation of a parked `accept`, reaped like any other completion.
    ...
    pub fn deinit(self: *Server) void {
        // The listener, unless `close_to_new` already gave it back.
    ...
    /// Stop taking connections, and give the port back.
    ///
    /// The flag alone is not enough. An `accept` is nearly always outstanding in
    /// the ring, and a pending io_uring request holds a reference to the file it
    /// names — so a listening socket with an accept parked on it outlives the
    /// process by however long the kernel takes to cancel that request when the
    /// ring is torn down, about a second here. `SO_REUSEADDR` does not help: the
    /// socket is still LISTEN, not TIME_WAIT. The next server to start gets
    /// `AddressInUse`, which during a rolling restart is the whole point of
    /// stopping gracefully lost.
    ///
    /// So the parked accept is cancelled here, while there is still a loop running
    /// to reap it, and the socket is closed once it has been. Closing the
    /// descriptor first would not do: io_uring holds its own reference to the
    /// file, so the socket would stay LISTEN with nothing left to cancel it but
    /// tearing the ring down.
    ///
    /// `gave_the_port_back` is what a shutdown waits for.
    pub fn close_to_new(self: *Server) void {
        if (self.closing) return;
        self.closing = true;
        if (self.accepting) {
            self.cancel_completion = .{ .callback = on_cancelled, .context = self };
            self.loop.io.cancel(&self.cancel_completion, &self.accept_completion) catch {
                // Nothing to do but close it and let the ring's teardown deal
                // with what is parked; the port comes back a moment later.
                self.close_listener();
            };
            return;
        }
    ...
    /// Has the listening socket been closed, with nothing left parked on it?
    pub fn gave_the_port_back(self: *const Server) bool {
        return self.listener_closed;
    }

    fn close_listener(self: *Server) void {
        if (self.listener_closed) return;
        self.listener_closed = true;
        posix.close(self.listener);
    }

    fn on_cancelled(completion: *Completion) void {
        const self: *Server = @ptrCast(@alignCast(completion.context.?));
        // The accept it names comes back on its own; the socket is closed there.
        // Unless it was already gone, in which case there is nothing to wait for.
        if (!self.accepting) self.close_listener();
    }
    ...
    fn on_accept(completion: *Completion) void {
        const self: *Server = @ptrCast(@alignCast(completion.context.?));
        self.accepting = false;
        // Whatever it came back as, nothing is parked on the listener now, so the
        // port can go back.
        if (self.closing) self.close_listener();
...
test "a graceful stop gives the port back before the process leaves" {
    // The bug this is here for: `accept_more` leaves an accept parked in the
    // ring, io_uring holds its own reference to the file that accept names, and
    // closing the descriptor does not drop it. The socket stayed LISTEN for about
    // a second after the process was gone, and a server restarted into that
    // window died with `AddressInUse` — which is exactly the rolling restart the
    // graceful stop exists for.
    var loop = io_mod.Loop.init(testing.allocator) catch return error.SkipZigTest;
    defer loop.deinit();

    const wildcard = try std.net.Address.parseIp("127.0.0.1", 0);
    const listener = try io_mod.listen(wildcard, 32);
    const port = try io_mod.bound_port(listener);
    var echo = Echo{};
    var server = try Server.init(testing.allocator, &loop, listener, echo.handler(), 4);
    defer server.deinit();

    server.accept_more();
    try testing.expect(server.accepting);
    try testing.expect(!server.gave_the_port_back());

    // The accept has to actually reach the kernel: `accept_more` only queues the
    // submission, and a queued one holds nothing. Without this the test passes
    // against the defect it exists for.
    try loop.tick();
    try testing.expect(server.accepting);

    server.close_to_new();
    // The cancellation and the accept it names both have to be reaped, so the
    // loop has to run. It takes a tick or two, not a second.
    var ticks: u32 = 0;
    while (!server.gave_the_port_back() and ticks < 64) : (ticks += 1) {
        loop.tick() catch break;
    }
    try testing.expect(server.gave_the_port_back());

    // The whole point, and the thing that used to fail: the port is bindable
    // again, here, now, with the ring still up.
    const again = try io_mod.listen(try std.net.Address.parseIp("127.0.0.1", port), 32);
    posix.close(again);
}
