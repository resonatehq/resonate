...
pub const IO = struct {
    ...
    /// Ask the kernel to cancel a request that is already submitted.
    ///
    /// The cancellation is itself a request: `completion` is reaped for it, and
    /// the request it names comes back separately, with `ECANCELED` if it was
    /// still parked. Needed at shutdown to get a listening socket out of the ring:
    /// a pending request holds its own reference to the file it names, so closing
    /// the descriptor is not enough — an `accept` parked on a listener keeps that
    /// socket LISTEN until the ring is torn down, and the next process to start
    /// cannot bind the port.
    pub fn cancel(self: *IO, completion: *Completion, target: *Completion) Error!void {
        const s = try self.sqe();
        s.prep_cancel(@intFromPtr(target), 0);
        s.user_data = @intFromPtr(completion);
        self.in_flight += 1;
    }
