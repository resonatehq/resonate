...
async function chargeV1(_ctx: Context, amount: number): Promise<number> {
  return amount;
}

// v2: the billing rules changed -- add a 3% processing fee.
async function chargeV2(_ctx: Context, amount: number): Promise<number> {
...
try {
  ...
  const v1 = await resonate.run(`charge-run-v1-${ts}`, chargeV1, 100);
  const v2 = await resonate.run(`charge-run-v2-${ts}`, chargeV2, 100);
  ...
  const rpcV1 = await resonate.rpc(`charge-rpc-v1-${ts}`, "charge", 100);
  const rpcV2 = await resonate.rpc(`charge-rpc-v2-${ts}`, "charge", 100, resonate.options({ version: 2 }));
