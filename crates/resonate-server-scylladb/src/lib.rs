...
/// Read `[servers.server_scylladb]` and build the server. Nothing is opened
/// here — the session is `init`'s, like every other port's resource.
fn configure(
    settings: &Settings<'_>,
    ...
    if config.shards < 1 {
        return Err(settings.reject("shards", "must be at least 1"));
    }
