//! The Resonate CLI client: `promises`, `tasks`, `schedules`, `invoke`, `tree`,
//! and the MCP server.
//!
//! It talks to a Resonate server over HTTP and shares no code with one — no
//! plugin, no engine, no port trait. It is here rather than in the server's
//! binary because the server's binary is a composition root: a list of plugins
//! and a `run`, and this is neither.
