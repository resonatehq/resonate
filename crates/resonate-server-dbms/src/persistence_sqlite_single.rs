...
//!
//! Ten tables become six. `callbacks` and `listeners` stay relational, and so
//! do the two outgoing tables: the Postgres design folds the first two into
//! TEXT[] columns and the last two into an `outbox`, but SQLite has no array
//! type — 43 array operations have no equivalent here that is not a worse
//! JSON one — and with the arrays gone there is nothing for one outbox table
//! to simplify.
//!
...
//! consulted.
//!
//! One case the derivation does not reproduce exactly, inherited from the
//! Postgres single-table design: the multi-table `task.create` inserts a
//! `promise_timeouts` row for any promise it creates, targeted or not, while
//! `state = 'pending' AND target IS NOT NULL` puts only targeted ones on the
//! eager sweep. An untargeted `task.create` promise therefore times out
//! lazily here — through `try_timeout`, the way an untargeted `promise.create`
//! already does in every backend — rather than through `process_timeouts`.
...
// ---------------------------------------------------------------------------
// The four transformations, for anyone diffing this against
