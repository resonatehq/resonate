//! Single-table SQLite backend — INCOMPLETE, not wired to anything.
//!
//! The schema below is finished and is the collapse; the method bodies are
//! still the multi-table ones, copied verbatim, and reference four tables this
//! schema no longer creates. Nothing constructs this type, so it cannot be
//! reached — see the handoff note at the bottom of this file for what remains.
//!
//!
//! The same collapse the Postgres single-table design makes, minus the part
//! SQLite cannot express. A task is columns on its promise, and the timeout
//! queues are derivable rather than stored:
//!
//!   tasks           -> task_state, task_version
//!   task_timeouts   -> retry_at, expires_at, ttl, pid
//!   promise_timeouts-> derivable: state = 'pending' AND timeout_at <= now
//!   schedule_timeouts -> schedules.next_run_at
//!   outgoing_*      -> one outbox table
//!
//! Ten tables become five. `callbacks` and `listeners` stay relational: the
//! Postgres design folds them into TEXT[] columns, and SQLite has no array
//! type — 43 array operations have no equivalent here that is not a worse
//! JSON one.
...
pub struct SqliteSingleStorage {
    conn: Arc<Mutex<Connection>>,
    task_retry_timeout: i64,
}

impl SqliteSingleStorage {
...
impl<'a> Db for SqliteSingleDb<'a> {
...
// ---------------------------------------------------------------------------
//
// 20 of the 34 Db methods still address tables the schema above no longer
// creates: 101 references in total, none of them safely mechanical.
//
//     9  process_timeouts          4  task_release       2  task_acquire
//     9  snap                      4  task_halt          2  task_suspend
//     7  task_heartbeat            4  task_continue      1  lock_for_update
//     6  process_schedule_timeout  3  task_create        1  try_timeout
//     4  promise_create            3  task_fulfill       1  task_get
//     4  promise_register_callback 3  debug_reset        1  task_search
//                                                        1  schedule_create
//                                                        1  get_expired_schedule_timeouts
//
...
//                     ttl become pid and ttl on the promise.
//
//   promise_timeouts  Delete every reference. The queue is
//                     `state = 'pending' AND timeout_at <= ?`, which is what
//                     idx_promises_timeout_at indexes. Inserts and deletes
//                     against the table simply go away.
//
//   schedule_timeouts Same: `next_run_at <= ?` on schedules.
//
// Start with promise_create and task_create — they establish the NULL-task
// convention every other method depends on. Leave process_timeouts and snap for
// last: the first is the three-statement sweep, and the second is what the
// differential compares byte-for-byte, so a mistake there shows up as every
// operation diverging rather than one.
//
// To wire it up: add a PostgresSingle-style variant to Storage, register it in
// the differential beside "sqlite", and iterate with
//
//     TEST_BACKENDS=sqlite,sqlite-single cargo test --test differential -- --nocapture
//
// Then repeat the whole exercise for MySQL.
