//! Single-table MySQL backend.
//!
//! The same collapse as `persistence_sqlite_single.rs`, in MySQL. A task is
//! columns on its promise, and the timeout queues are derivable rather than
//! stored:
//!
...
//!
//! Ten tables become six. `callbacks`, `listeners` and the two outgoing tables
//! stay relational, as in the SQLite collapse. `persistence_sqlite_single.rs`
//! documents what a derived queue costs, including the one case the derivation
//! does not reproduce exactly — an untargeted `task.create` promise times out
//! lazily here rather than through the eager sweep.
//!
//! # Why this needs its own database
//!
//! The Postgres single-table backend lives in schema `resonate` and the
//! multi-table one in `public`, so both can share a database. MySQL's schema
//! *is* its database, so the two backends cannot share one without sharing
//! their `promises` table — which for a differential would be comparing a
//! backend against itself. Point this one at a second database (the
//! differential uses `resonate_single`); everything else about the connection
//! is the same.
...
/// - `MysqlSingleDb` is created and dropped within a single `block_in_place` call
/// - Only one `&MysqlSingleDb` reference exists at a time (no aliasing)
/// - The `UnsafeCell` is never accessed concurrently
struct MysqlSingleDb<'a> {
    tx: UnsafeCell<sqlx::Transaction<'a, sqlx::MySql>>,
    task_retry_timeout: i64,
}

impl<'a> MysqlSingleDb<'a> {
    /// Returns a mutable reference to the underlying transaction.
    ///
    /// # Safety
    /// Safe because `MysqlSingleDb` is only used within a single synchronous closure
