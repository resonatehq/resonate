//! The server's place in the ring, allocated before the server exists.
...
/// hold a `Weak` to.
///
/// The ring needs the server's handle before the server can be built: a worker
/// holds one, and the router that holds the workers is what the server is
/// constructed *from*. With a single concrete server type that is
/// [`Arc::new_cyclic`] — the framework allocates the `Arc`, so it knows the
/// type, and hands out the `Weak` before the value exists.
///
/// Once the server is a plugin, the framework no longer knows the type. So the
/// place in the ring is allocated instead of the server: workers and gateways
/// hold this, a plugin builds whatever it likes behind it — including its own
/// `new_cyclic`, which an engine-backed server needs for its timer — and
/// [`fulfil`](Self::fulfil) closes the ring.
///
...
impl ServerHandle {
    ...
    /// Put the server in its place. The second call is refused rather than
    /// ignored: two servers behind one handle is a wiring bug, and the one
    /// that lost would be silently unreachable.
    pub fn fulfil(&self, server: Arc<dyn ResonateServer>) -> Result<(), &'static str> {
        self.inner
            .set(server)
            .map_err(|_| "the ring was already closed: this handle already holds a server")
    }
    ...
    fn server(&self) -> Result<&Arc<dyn ResonateServer>, Unavailable> {
        ...
        // workers are started after the server is in place — so this is
        // unreachable in a running process. It is an error rather than a panic
        // because a wiring mistake should be reported, not abort.
        self.inner
            .get()
...
mod tests {
    ...
    #[tokio::test]
    async fn a_handle_is_not_ready_until_the_ring_closes() {
