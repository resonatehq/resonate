...
func (v *Validator) ValidateCreateSuscription(model *Model, reqTime int64, resTime int64, req *t_api.Request, res *t_api.Response) (*Model, error) {
	p := model.promises.get(req.CreateSuscription.PromiseId)

	switch res.CreateSuscription.Status {
	case t_api.StatusCreated:
		if p == nil {
			return model, fmt.Errorf("promise '%s' does not exist", req.CreateSuscription.PromiseId)
		}

		if p.State != promise.Pending {
			return model, fmt.Errorf("promise '%s' must be pending", req.CreateSuscription.PromiseId)
		}

		if model.callbacks.get(res.CreateSuscription.Callback.Id) != nil {
			return model, fmt.Errorf("suscription '%s' exists", res.CreateSuscription.Callback.Id)
		}

		model = model.Copy()
		model.callbacks.set(res.CreateSuscription.Callback.Id, res.CreateSuscription.Callback)
		return model, nil
	...
	case t_api.StatusOK:
		...
		if resTime >= p.Timeout {
			model = model.Copy()
			model.promises.set(p.Id, res.CreateSuscription.Promise)
			return model, nil
		}

		// otherwise verify the suscription was created previously
		suscriptionId := fmt.Sprintf("%s.%s", p.Id, req.CreateSuscription.Id)
		if model.callbacks.get(suscriptionId) == nil {
			return model, fmt.Errorf("suscription '%s' must exist", suscriptionId)
		}
	...
	case t_api.StatusPromiseNotFound:
		if p != nil {
			return model, fmt.Errorf("promise '%s' exists", req.CreateSuscription.PromiseId)
		}
		return model, nil

	default:
		return model, fmt.Errorf("unexpected response status '%d'", res.CreateSuscription.Status)
	}
}
