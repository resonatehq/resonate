...
// That is the same trick `recordFrom` plays in valid/validator.lean.
//
...
// The Lean decoder's task names, from valid/json.lean's `taskState`.
...
// shape capture.py tees, so the Lean checker reads it unmodified.
//
// Records are emitted in FULL, because `valid/json.lean` decodes full
