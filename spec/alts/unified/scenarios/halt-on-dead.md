# Scenario: halt-on-dead

Model-generated from the counterexample that violates `NoHaltOnDead`.
...
| 3 | `task.halt id=a` | **this is the call that must be refused** |

**Violates** `NoHaltOnDead` at step 3.

The recorded prefix is in `traces/gen-halt-on-dead.ndjson`; it stops
