...
def stepMutants : List (String × Bool) :=
  ...
  [ ("preserved_promise_birth_fields_immutable",
    ...
    ("consistent_task_state_edge_admissible",
       consistent_task_state_edge_admissible 0 { objects := [objWith (oid "a") PT (T)] } { objects := [objWith (oid "a") PT ({ T with state := .suspended, retryAt := none })] }),
    ("preserved_task_acquisition_only_from_pending",
       preserved_task_acquisition_only_from_pending 0 { objects := [objWith (oid "a") PT ({ T with state := .suspended, retryAt := none })] } { objects := [objWith (oid "a") PT (A)] }),
    ("preserved_task_suspension_only_from_acquired",
       preserved_task_suspension_only_from_acquired 0 { objects := [objWith (oid "a") PT (T)] } { objects := [objWith (oid "a") PT ({ T with state := .suspended, retryAt := none })] }),
    ("preserved_task_halted_only_reenters_via_pending",
       preserved_task_halted_only_reenters_via_pending 0 { objects := [objWith (oid "a") PT ({ T with state := .halted, retryAt := none })] } { objects := [objWith (oid "a") PT ({ T with state := .suspended, retryAt := none })] }),
    ...
    ("consistent_task_fulfilment_needs_settlement",
       consistent_task_fulfilment_needs_settlement 0 { objects := [objWith (oid "a") (PT) (T)] } { objects := [objWith (oid "a") (PT) ({ T with state := .fulfilled, retryAt := none })] }),
    ...
    ("consistent_wake_follows_callback_consumption",
       consistent_wake_follows_callback_consumption 0
         { objects := [objOf (oid "c") ({ P with callbacks := [oid "a"] }), objWith (oid "a") PT ({ T with state := .suspended, retryAt := none })] }
         { objects := [objOf (oid "c") ({ P with callbacks := [oid "a"] }), objWith (oid "a") PT ({ T with state := .pending, retryAt := some 30 })] }),
    ("consistent_suspension_registers_callback",
       consistent_suspension_registers_callback 0 { objects := [objWith (oid "a") (P) (A)] } { objects := [objWith (oid "a") (P) ({ A with state := .suspended, pid := none, ttl := none, expiresAt := none })] }),
    ...
    ("consistent_task_birth_state",
       consistent_task_birth_state 0 { } { objects := [objWith (oid "a") PT ({ T with state := .suspended, retryAt := none })] }),
    ...
    ("consistent_task_lease_deadline_is_now_plus_ttl",
       consistent_task_lease_deadline_is_now_plus_ttl 40 { objects := [objWith (oid "a") PT (A)] } { objects := [objWith (oid "a") PT ({ A with expiresAt := some 999 })] }),
    ("consistent_task_pending_entry_arms_retry",
       consistent_task_pending_entry_arms_retry 7 { objects := [objWith (oid "a") PT (A)] } { objects := [objWith (oid "a") PT ({ T with version := 4, retryAt := none })] }),
    ("consistent_task_retry_rearm_only_when_due",
       consistent_task_retry_rearm_only_when_due 5 { objects := [objWith (oid "a") PT ({ T with retryAt := some 50 })] } { objects := [objWith (oid "a") PT ({ T with retryAt := some 900 })] }),
    ("consistent_task_wake_records_resume",
       consistent_task_wake_records_resume 7 { objects := [objWith (oid "a") PT ({ T with state := .suspended, retryAt := none })] } { objects := [objWith (oid "a") PT ({ T with retryAt := some 7 })] }),
    ...
    ("preserved_no_dead_dispatch",
       preserved_no_dead_dispatch 500
         { objects := [objWith (oid "a") (P) ({ T with state := .acquired, pid := some "w", ttl := some 1, expiresAt := some 1, retryAt := none })] }
