...
#
# Every other check in `zig/` grades this server against a specification written
# in this repository — the simulator's search replays the same state machine, and
# the differential compares against the Rust tree. Agreement there is agreement
# with ourselves. So this one hands a recorded history to Porcupine, driven by the
# model in `resonatehq/resonate-specification`, which nobody here wrote.
#
...
#
# Usage: zig/tools/against-the-go-checker.sh [seeds]
#
# Needs `go`, a `conctrace` built from the Rust tree
# (`cargo build --release --example conctrace`), and this directory's own
# binaries. It clones the specification repository unless SPEC_DIR points at one.
...
# the same from the repository root and from `zig/`.
here="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
zig_root="$(dirname "$here")"
repo_root="$(dirname "$zig_root")"
bin="${BIN:-$zig_root/zig-out/bin}"
conctrace="${CONCTRACE:-$repo_root/target/release/examples/conctrace}"
work="$(mktemp -d)"
...
# ── The checker, from the repository that owns the specification ──────────────
spec="${SPEC_DIR:-$work/spec}"
if [[ ! -d "$spec/valid/porc" ]]; then
  GIT_LFS_SKIP_SMUDGE=1 git clone --depth 1 -q \
    ...
    fail "could not clone the specification repository"
fi
